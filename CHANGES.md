...
## 2026-06-15 01:25 — Исправление: скрипты в head игнорируются при HTMX swap

**Корневая причина:** скрипты регистрации Alpine.js компонентов в `players/detail.html` и `tournaments/detail.html` находились в `{% block extra_head %}` (т.е. в `<head>`). Документация HTMX: *"only the body of the web page will be updated, so any styles and scripts in the new page head tag will be discarded"*. При HTMX swap скрипты в `<head>` не выполняются → компоненты не зарегистрированы → пустая страница.

**Исправление:** перенесены скрипты из `{% block extra_head %}` в `{% block content %}` (в `<body>`) для `players/detail.html` и `tournaments/detail.html`. HTMX повторно выполняет `<script>` теги в swap-нутом `<body>`-контенте.

**Верификация:** протестировано через Playwright MCP (headless Chromium): edit → "Отмена" → detail загружается корректно. Все API-запросы 200 OK.

- 160/160 тестов проходят, ruff clean

## 2026-06-18 01:55

**Создан/обновлён файл:** REPORT_HUMAN.md
