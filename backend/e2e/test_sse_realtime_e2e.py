...
def test_sse_notification_toast_appears(page: Page, server_url: str, admin_token: str):
...
def test_sse_client_has_reconnect_method(server_url: str):
    ...
    sse_js_path = os.path.join(
        os.path.dirname(__file__), "..", "app", "static", "js", "sse.js"
    )
    with open(sse_js_path) as f:
        content = f.read()
...
def test_dashboard_has_game_sse_listeners(server_url: str, admin_token: str):
...
def test_players_page_has_sse_listeners(server_url: str, admin_token: str):
...
def test_tournament_detail_has_sse_listeners(server_url: str, admin_token: str):
